...
const AIInsights: React.FC = () => {
  ...
  const generateComprehensiveInsights = async (
    ...
    if (metrics.winRate > 65) {
      generatedInsights.push({
        ...
        description: `Your ${metrics.winRate.toFixed(
          1
        )}% win rate is outstanding. You're demonstrating excellent market timing and entry selection.`,
        actionable:
          "Consider increasing position sizes on your highest conviction setups to maximize this edge.",
        icon: Target,
        confidence: 90,
        priority: "high",
      });
      ...
      generatedInsights.push({
        ...
        description: `Your ${metrics.winRate.toFixed(
          1
        )}% win rate suggests systematic issues with trade selection or timing.`,
        actionable:
          "Review your entry criteria and consider paper trading new setups before implementing them live.",
        icon: AlertTriangle,
        confidence: 95,
        priority: "high",
      });
    ...
    if (metrics.avgRiskReward > 2.5) {
      generatedInsights.push({
        ...
        description: `Your ${metrics.avgRiskReward.toFixed(
          2
        )}:1 risk-reward ratio demonstrates exceptional discipline. This gives you a significant mathematical edge.`,
        actionable:
          "Your risk management is excellent. Focus on maintaining this discipline while potentially increasing trade frequency.",
        icon: TrendingUp,
        confidence: 85,
        priority: "medium",
      });
      ...
      generatedInsights.push({
        ...
        description: `Your ${metrics.avgRiskReward.toFixed(
          2
        )}:1 ratio is mathematically unsustainable long-term.`,
        actionable:
          "Immediately implement minimum 2:1 risk-reward rule. Avoid trades that don't meet this criteria.",
        icon: AlertTriangle,
        confidence: 98,
        priority: "high",
      });
    ...
    if (metrics.consistency > 70) {
      generatedInsights.push({
        ...
        description: `${metrics.consistency.toFixed(
          1
        )}% of your months are profitable, indicating excellent emotional control and systematic approach.`,
        actionable:
          "Your consistency is excellent. Document your process to maintain this edge during difficult periods.",
        icon: Activity,
        confidence: 80,
        priority: "medium",
      });
      ...
      generatedInsights.push({
        ...
        description: `Only ${metrics.consistency.toFixed(
          1
        )}% of your months are profitable, suggesting emotional or systematic inconsistencies.`,
        actionable:
          "Implement strict rules for position sizing and risk management. Consider reducing trade frequency to focus on quality.",
        icon: Brain,
        confidence: 75,
        priority: "high",
      });
    ...
    if (patterns.revengeTrading) {
      generatedInsights.push({
        ...
        description:
          "AI detected increased trading frequency and size after significant losses, indicating emotional decision-making.",
        actionable:
          "Implement mandatory 24-hour cooling off period after any loss exceeding 2% of account. Use alerts to enforce this rule.",
        icon: Brain,
        confidence: 80,
        priority: "high",
      });
    ...
    if (patterns.weekendGap) {
      generatedInsights.push({
        ...
        description:
          "Your trades show better performance when entered on Monday mornings, likely capturing weekend gap moves.",
        actionable:
          "Consider allocating more capital to Monday morning setups and reducing Friday afternoon positions.",
        icon: Clock,
        confidence: 65,
        priority: "medium",
      });
    ...
    if (symbolAnalysis.specialist) {
      generatedInsights.push({
        ...
        description: `Your ${symbolAnalysis.winRate.toFixed(1)}% win rate on ${
          symbolAnalysis.topSymbol
        } significantly outperforms your overall average.`,
    ...
    if (marketConditions.trendingBias) {
      generatedInsights.push({
        ...
        description:
          "Your performance significantly improves during trending market conditions versus ranging markets.",
        actionable:
          "Focus trading during high volatility periods and reduce activity during low volatility consolidation phases.",
        icon: TrendingUp,
        confidence: 60,
        priority: "low",
      });
    ...
    if (metrics.profitFactor > 2.0) {
      generatedInsights.push({
        ...
        description: `Your ${metrics.profitFactor.toFixed(
          2
        )} profit factor indicates you're generating excellent returns relative to losses.`,
        actionable:
          "Your system is working excellently. Focus on scaling up gradually while maintaining the same discipline.",
        icon: Zap,
        confidence: 85,
        priority: "medium",
      });
      ...
      generatedInsights.push({
        ...
        description: `Your ${metrics.profitFactor.toFixed(
          2
        )} profit factor indicates you're barely breaking even after accounting for costs.`,
        actionable:
          "Immediately review and tighten entry criteria. Consider reducing position sizes until profitability improves.",
        icon: AlertTriangle,
        confidence: 92,
        priority: "high",
      });
    ...
    if (metrics.maxDrawdown > 1000) {
      ...
      generatedInsights.push({
        ...
        description: `Your maximum drawdown of $${metrics.maxDrawdown.toFixed(
          2
        )} represents significant capital at risk. ${
          recentDrawdownTrend
            ? "Recent trades show concerning drawdown acceleration."
            : "Current drawdown levels appear stable."
        }`,
    ...
    if (frequencyAnalysis.overtrading) {
      generatedInsights.push({
        ...
        actionable:
          "Implement a maximum of 3 trades per day rule. Quality over quantity will improve your performance.",
        icon: Activity,
        confidence: 75,
        priority: "medium",
      });
    ...
    if (emotionalState.highRisk) {
      generatedInsights.push({
        ...
        actionable:
          "Consider taking a 48-hour break from trading. Review and document your trading rules before resuming.",
        icon: Brain,
        confidence: 82,
        priority: "high",
      });
  ...
  const analyzeAdvancedPatterns = (trades: TradeData[]) => {
    ...
    for (let i = 1; i < sortedTrades.length; i++) {
      ...
      if (prevTrade.profit < -100) {
        // Significant loss
        const timeDiff =
          new Date(currentTrade.date).getTime() -
          new Date(prevTrade.date).getTime();
        const withinHours = timeDiff < 4 * 60 * 60 * 1000; // Within 4 hours

        if (
          withinHours &&
          Math.abs(currentTrade.quantity) > Math.abs(prevTrade.quantity) * 1.5
        ) {
          revengeTrading = true;
          break;
        }
    ...
    const mondayTrades = trades.filter(
      (trade) => new Date(trade.date).getDay() === 1
    );
    const otherDayTrades = trades.filter(
      (trade) => new Date(trade.date).getDay() !== 1
    );

    if (mondayTrades.length > 5 && otherDayTrades.length > 5) {
      const mondayAvg =
        mondayTrades.reduce((sum, t) => sum + t.profit, 0) /
        mondayTrades.length;
      const otherAvg =
        otherDayTrades.reduce((sum, t) => sum + t.profit, 0) /
        otherDayTrades.length;
  ...
  const analyzeSymbolSpecialization = (trades: TradeData[]) => {
    ...
    Object.entries(symbolStats).forEach(([symbol, stats]) => {
      ...
      if (stats.total >= 10 && winRate > bestWinRate) {
        // At least 10 trades
        topSymbol = symbol;
        bestWinRate = winRate;
      }
  ...
  const analyzeMarketConditions = (trades: TradeData[]) => {
    ...
    const avgProfit =
      recentTrades.reduce((sum, t) => sum + t.profit, 0) / recentTrades.length;
  ...
  const analyzeFrequencyPatterns = (trades: TradeData[]) => {
    const tradingDays = new Set(
      trades.map((t) => new Date(t.date).toDateString())
    ).size;
  ...
  const analyzeTimeBasedPerformance = (trades: TradeData[]) => {
    ...
    Object.entries(timeWindows).forEach(([window, windowTrades]) => {
      if (windowTrades.length >= 5) {
        const windowAvg =
          windowTrades.reduce((sum, t) => sum + t.profit, 0) /
          windowTrades.length;
    ...
    const improvement = bestWindow
      ? ((bestPerformance - overallAvg) / Math.abs(overallAvg)) * 100
      : 0;
  ...
  const predictEmotionalTradingRisk = (trades: TradeData[]) => {
    ...
    for (let i = 1; i < recentTrades.length; i++) {
      ...
      if (
        previous.profit < 0 &&
        Math.abs(current.quantity) > Math.abs(previous.quantity) * 1.3
      ) {
        riskScore += 25;
      }
    }
    ...
    for (let i = 1; i < Math.min(recentTrades.length, 5); i++) {
      const timeDiff =
        new Date(recentTrades[i - 1].date).getTime() -
        new Date(recentTrades[i].date).getTime();
  ...
  const generatePerformancePrediction = (
    ...
    const expectedChange =
      score > 60 ? "improved" : score < 40 ? "challenging" : "stable";
...
`;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      try {
        ...
        aiResponse =
          "I'm having trouble processing your question right now, but I'm here to help! Could you try rephrasing your question or ask me about your trading performance, risk management, or any specific patterns you've noticed?";
      }
  ...
  const startNewConversation = () => {
    const welcomeMessage: ChatMessage = {
      id: Date.now().toString(),
      content: `Hi Mike! I'm your personal trading AI coach. I've analyzed your ${
        activeStrategy?.name || "current"
      } strategy with ${
        trades.length
      } trades.\n\n🎯 **Quick Stats**: ${metrics?.winRate.toFixed(
        1
      )}% win rate, ${metrics?.avgRiskReward.toFixed(
        2
  ...
  const getTopRecommendations = (): string[] => {
    ...
    if (metrics.avgRiskReward < 1.5) {
      recommendations.push(
        "Improve your risk-reward ratio by targeting 2:1 minimum on all trades"
      );
    }

    if (metrics.winRate < 45) {
      recommendations.push(
        "Focus on trade quality over quantity - tighten your entry criteria"
      );
    }

    if (metrics.maxDrawdown > 1500) {
      recommendations.push(
        "Implement stricter stop-loss rules to reduce maximum drawdown"
      );
    }

    if (metrics.consistency < 50) {
      recommendations.push(
        "Work on emotional discipline and consistent position sizing"
      );
    }

    if (predictEmotionalTradingRisk(trades).highRisk) {
      recommendations.push(
        "Take a trading break to reset emotional state and review rules"
      );
    }
  ...
  const generateFallbackResponse = (
    ...
    if (
      ...
      if (lastUserMessage.includes("pattern")) {
        return `Let me dive deeper into your trading patterns:\n\n🕐 **Time Analysis**: ${
          analyzeTimeBasedPerformance(trades).bestTimeWindow
            ? `You perform ${
                analyzeTimeBasedPerformance(trades).improvement
              }% better during ${
                analyzeTimeBasedPerformance(trades).bestTimeWindow
              }`
            : "No clear time-based patterns detected yet"
        }\n\n📈 **Symbol Performance**: ${
          analyzeSymbolSpecialization(trades).specialist
            ? `You're particularly strong with ${
                analyzeSymbolSpecialization(trades).topSymbol
              } (${analyzeSymbolSpecialization(trades).winRate.toFixed(
                1
              )}% win rate)`
            : "Performance seems consistent across different symbols"
        }\n\n🧠 **Psychology**: ${
          predictEmotionalTradingRisk(trades).highRisk
            ? "I detect elevated emotional trading risk - consider taking a break"
            : "Your emotional control is stable"
      ...
      if (lastUserMessage.includes("improve")) {
        ...
        return `Here are specific steps you can take this week:\n\n${recommendations
          .map((rec, i) => `**${i + 1}. ${rec.split(" ")[0]} Focus**: ${rec}`)
          .join(
            "\n\n"
    ...
    if (
      ...
      const responses = [
        ...
        patterns.revengeTrading
          ? "• ⚠️ **Revenge Trading Detected** - You tend to increase position sizes after losses"
          : "• ✅ **Emotional Control** - No revenge trading patterns detected",
        timeAnalysis.bestTimeWindow
          ? `• ⏰ **Peak Performance**: ${timeAnalysis.bestTimeWindow} (${timeAnalysis.improvement}% better)`
          : "",
    ...
    if (
      ...
      if (predictEmotionalTradingRisk(trades).highRisk)
        urgentIssues.push("emotional control");

      if (urgentIssues.length > 0) {
        return `🚨 **Priority Alert**: Your ${urgentIssues.join(
          " and "
        )} need immediate attention.\n\n**This Week's Action Plan**:\n1. ${topRecommendation}\n\n**Quick Win**: ${
          urgentIssues[0] === "risk-reward ratio"
            ? "Set a rule to only take trades with 2:1 reward potential"
            : urgentIssues[0] === "entry criteria"
            ? "Paper trade for 3 days to refine your setup criteria"
            : "Take a 24-hour break after any loss over $100"
    ...
    if (
      ...
      lowerQuestion.includes("drawdown") ||
      ...
      return `**Risk Assessment for Your Trading**:\n\n📊 **Current Stats**:\n• Max Drawdown: $${metrics.maxDrawdown.toFixed(
        2
      )}\n• Risk-Reward: ${metrics.avgRiskReward.toFixed(
        2
      )}:1\n• Overall Grade: **${riskGrade}**\n\n${
        riskGrade < "B"
          ? "🚨 **Action Required**: Your risk management needs improvement. Consider reducing position sizes by 50% until you achieve consistent 2:1 risk-reward ratios."
          : "✅ **Good Foundation**: Your risk management is solid. Focus on consistency."
      }\n\nWhat's your biggest concern about risk right now?`;
    }

    if (
      lowerQuestion.includes("psychology") ||
      lowerQuestion.includes("emotion") ||
      lowerQuestion.includes("feeling") ||
      lowerQuestion.includes("revenge")
    ) {
      ...
      return `**Trading Psychology Analysis**:\n\n🧠 **Mental State**: ${
        emotionalRisk.highRisk
          ? "⚠️ High emotional risk detected"
          : "✅ Stable emotional control"
      }\n📈 **Consistency**: ${metrics.consistency.toFixed(
        1
      )}% of months profitable\n🎯 **Grade**: **${psychGrade}**\n\n${
        emotionalRisk.highRisk
          ? "**Immediate Action**: Take a 48-hour trading break. Review your rules and come back refreshed."
          : "**Strength**: You maintain good emotional discipline. Keep following your systematic approach."
      ...
      return `**Timing Analysis**:\n\n${
        timeAnalysis.bestTimeWindow
          ? `🕐 **Your Peak Hours**: ${
            ...
            }% better than average\n\n💡 **Strategy**: Focus your highest-conviction trades during ${timeAnalysis.bestTimeWindow.toLowerCase()} and reduce activity outside this window.`
          : `⏰ I don't see clear time-based patterns yet. Try tracking your mood and market conditions during different trading sessions.`
    ...
    return `Great question! With ${
      trades.length
    } trades and a ${metrics.winRate.toFixed(
      1
    )}% win rate, you're building solid experience.\n\n💡 **Today's Insight**: ${
    ...
    <div className="max-w-7xl mx-4 px-2 sm:px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-400 mb-6">
          Advanced AI analysis of your {activeStrategy?.name || "trading"}{" "}
          strategies performance and behavior patterns
        </p>
        ...
        {metrics && (
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
            ...
            <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
              ...
              <div className="text-2xl font-bold text-white">
                {metrics.avgRiskReward.toFixed(1)}:1
              </div>
            </div>
            <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
              <div className="text-xs text-gray-400 uppercase tracking-wide">
      ...
      <div className="space-y-6">
          ...
          <div className="text-center py-12">
            <div className="animate-pulse">
              ...
              <p className="text-gray-400">
                Analyzing {trades.length} trades for deep insights...
              </p>
        ...
        {!isAnalyzing && sortedInsights.length === 0 && (
          <div className="text-center py-12 bg-gray-800 rounded-lg border border-gray-700">
            ...
            <p className="text-gray-400 mb-4">
              Complete more trades to unlock powerful AI insights
            </p>
            <p className="text-sm text-gray-500">
              Minimum 10 trades needed for basic analysis
            </p>
          </div>
        )}
        ...
        {sortedInsights.map((insight, index) => {
          ...
          return (
            <div
              key={index}
              className={`p-6 rounded-lg border ${getInsightColor(
                insight.type
              )} transition-all hover:scale-[1.01]`}
            >
              <div className="flex items-start gap-4">
                <div
                  className={`p-3 rounded-lg bg-gray-900/50 ${getIconColor(
                    insight.type
                  )}`}
                >
      ...
      {sortedInsights.length > 0 && (
        <div className="mt-8 p-6 bg-gray-800 rounded-lg border border-gray-700">
          ...
          <p className="text-gray-300 text-sm">
            Based on analysis of {trades.length} trades from your{" "}
            {activeStrategy?.name || "current"} strategy. Insights are ranked by
            priority and confidence level. High-priority items require immediate
            attention, while medium and low priority insights can be implemented
            gradually for optimization.
          </p>
          <div className="mt-4 text-xs text-gray-500">
            Last updated: {new Date().toLocaleString()} • Next analysis: Live
            ...
            <div className="p-4 border-t border-gray-700">
              ...
              {chatMessages.length <= 1 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    onClick={() =>
                      setInputMessage("What patterns do you see in my trading?")
                    }
                  ...
                  <button
                    onClick={() =>
                      setInputMessage("How can I improve my risk management?")
                    }
                    className="text-xs bg-gray-700 hover:bg-gray-600 text-gray-300 px-2 py-1 rounded transition-colors"
                  >
                  ...
                  <button
                    onClick={() =>
                      setInputMessage("What's my biggest weakness right now?")
                    }
                    className="text-xs bg-gray-700 hover:bg-gray-600 text-gray-300 px-2 py-1 rounded transition-colors"
                  >
                    Biggest weakness?
