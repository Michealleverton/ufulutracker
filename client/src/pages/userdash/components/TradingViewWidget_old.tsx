      ...
      if (widgetRef.current) {
        ...
        widget.onChartReady(() => {
          ...
          try {
            ...
            }, 3000); // Longer delay for chart to fully load

            // Smarter save triggers with longer debouncing
            ...
            const debouncedSave = (reason: string) => {
              ...
              }, 8000); // 8 second debounce to prevent conflicts
            };

            // Event listeners for chart changes (less aggressive)
            activeChart.onSymbolChanged().subscribe(null, () => {
              setTimeout(() => debouncedSave('symbol change'), 2000);
            });

            activeChart.onIntervalChanged().subscribe(null, () => {
              setTimeout(() => debouncedSave('interval change'), 2000);
            });

            // Periodic auto-save (less frequent to avoid conflicts)
            ...
            // Save on important page events only
            const handleBeforeUnload = () => {
              console.log('🔄 Page unloading - final save...');
              if (!isLoading) {
                try {
                  const chartState = widget.save();
                  chartStateManager.saveToLocalStorage(chartState);
                } catch (error) {
                  console.warn('⚠️ Final save failed:', error);
                }
              }
            };
            ...
            const handleVisibilityChange = () => {
              if (document.hidden && !isLoading) {
                console.log('👁️ Tab hidden - saving...');
                setTimeout(() => debouncedSave('tab hidden'), 1000);
              }
            };
            ...
            // Much less aggressive user interaction saving
            const chartContainer = document.getElementById('tradingview-widget');
            if (chartContainer) {
              let lastInteraction = 0;
              const handleUserInteraction = () => {
                const now = Date.now();
                if (now - lastInteraction > 30000) { // Only trigger every 30 seconds max
                  lastInteraction = now;
                  setTimeout(() => debouncedSave('user interaction'), 5000);
                }
              };
            ...
            return () => {
              ...
              if (chartContainer) {
                chartContainer.removeEventListener('mouseup', () => {});
                chartContainer.removeEventListener('touchend', () => {});
              }
            };
